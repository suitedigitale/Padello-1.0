...
import {
  ...
  formatDate,
  formatCurrency,
  RACKET_PRICE as INITIAL_RACKET_PRICE,
  INSTRUCTOR_PRICE as INITIAL_INSTRUCTOR_PRICE
} from './constants.ts';
import {
  Booking,
  BookingDraft,
...
useEffect(() => {
  ...
  const handleQuickSearch = () => {
    ...
    setChatSystemMessage(
      `Mio Signore, ho attivato la ricerca rapida! 🏰 Sto setacciando il Regno per trovarle uno sfidante degno. Ho già inviato un messo reale al gestore di ${selectedClub.name}.`
    );
  ...
  const handleConfirmBooking = (rackets: number, instructor: boolean, paymentMethod: PaymentMethod) => {
    ...
    const newBooking: Booking = {
      ...
      totalPrice: bookingDraft.court.price + racksToPrice(rackets, racketPrice) + (instructor ? instructorPrice : 0),
      paymentMethod,
  ...
  const dayBookings = useMemo(
    () => (selectedClub ? bookings.filter((b) => b.date === formatDate(selectedDate) && b.clubId === selectedClub.id) : []),
    [bookings, selectedDate, selectedClub]
  );

  const handleSlotClick = useCallback(
    (court: Court, slot: TimeSlot) => {
        ...
        if (existingBooking) {
          if (isAdmin) {
            if (confirm('Questo slot è occupato. Vuoi scambiare le due prenotazioni?')) {
              ...
              const targetExtrasPrice =
                existingBooking.extras.rackets * racketPrice + (existingBooking.extras.instructor ? instructorPrice : 0);
              const targetNewTotal = existingBooking.status === 'unavailable' ? 0 : targetNewBasePrice + targetExtrasPrice;

              const updatedSourceBooking = {
                ...sourceBooking,
              ...
              const updatedTargetBooking = {
                ...
                slotId: sourceBooking.slotId,
                clubId: sourceBooking.clubId,
                totalPrice: targetNewTotal,
              };
  ...
  const renderContent = () => {
    switch (currentView) {
      case 'bookings':
        return (
          <BookingList
            ...
            onDelete={(id) => setBookings((p) => p.filter((b) => b.id !== id))}
            onRecordResult={setResultBookingId}
            onToggleSearch={() => {}}
          />
        );
      ...
      case 'tournaments':
        return (
          <TournamentList
            tournaments={tournaments.filter((t) => t.clubId === selectedClub?.id)}
            clubs={clubs}
            user={user!}
            onBack={() => setCurrentView('home')}
            onSelect={(t) => {
      ...
      case 'explore':
        return (
          <Explore
            ...
            onJoinMatch={() => {}}
            onCheckNotification={(id) => setManagerNotifications((prev) => prev.filter((n) => n.id !== id))}
          />
        );
      ...
      case 'community':
        return (
          <Community
            ...
            onAskToPlay={(p) => {
              setIsChatOpen(true);
              setChatSystemMessage(`Messaggio per ${p.name}: Vorrei sfidarti a Padel! Quando saresti disponibile? ⚔️`);
              setTimeout(() => setChatSystemMessage(null), 1000);
            }}
      ...
      case 'tournament_detail': {
        ...
        return t ? (
          <TournamentDetail
            ...
            onUpdateMatch={handleUpdateTournamentMatch}
            clubCourts={courts.filter((c) => c.clubId === t.clubId)}
          />
        ) : null;
      }
      ...
      case 'admin_dashboard':
        return selectedClub ? (
          <AdminDashboard
            club={selectedClub}
            courts={clubCourts}
            clubBookings={bookings.filter((b) => b.clubId === selectedClub.id)}
            clubTournaments={tournaments.filter((t) => t.clubId === selectedClub.id)}
            racketPrice={racketPrice}
            instructorPrice={instructorPrice}
            onBack={() => setCurrentView('home')}
            onUpdateGlobalPrices={(r, i) => {
              setRacketPrice(r);
              setInstructorPrice(i);
            }}
            onAddCourt={() => setEditingCourt({} as any)}
            onEditCourt={setEditingCourt}
            onDeleteCourt={(id) => setCourts((p) => p.filter((c) => c.id !== id))}
      ...
      default:
        if (!selectedClub) {
          return <ClubList clubs={clubs} currentUser={user!} onSelect={(c) => setSelectedClub(c)} onAddClub={() => setIsCreatingClub(true)} />;
        }
            ...
            <main className="pt-6 space-y-8">
              <div className="flex flex-col md:flex-row gap-4 md:items-center justify-between">
                <div className="bg-white dark:bg-slate-800 p-1.5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex w-full md:w-auto md:min-w-[400px]">
                  <button
                    onClick={() => setBookingMode('match')}
                    className={`flex-1 py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${
                      bookingMode === 'match' ? 'bg-orange-500 text-white shadow-md' : 'text-slate-500 dark:text-slate-400'
                    }`}
                  >
                    <LayoutGrid size={18} />
                    Campo
                  ...
                  <button
                    onClick={() => setBookingMode('lesson')}
                    className={`flex-1 py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${
                      bookingMode === 'lesson' ? 'bg-orange-500 text-white shadow-md' : 'text-slate-500 dark:text-slate-400'
                    }`}
                  >
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 py-3 px-6 rounded-xl text-sm font-bold border transition-all ${
                    showOnlyAvailable ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700' : 'bg-white dark:bg-slate-800 text-slate-500'
                  }`}
                >
                  <Filter size={16} /> Disponibili
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {clubCourts.map((court) => (
                  <CourtCard
                    ...
                    isAdmin={user?.role !== 'player'}
                    isLessonMode={bookingMode === 'lesson'}
                    showAvailableOnly={showOnlyAvailable}
  ...
  if (!user) {
    if (showIntro) {
      return <IntroSlide onEnter={() => setShowIntro(false)} />;
    }
    ...
    if (showForgotPassword) {
      return (
        <ForgotPasswordForm
          ...
          onSend={(email) => {
            setToastMessage(`Istruzioni inviate a ${email} 📩`);
            setShowForgotPassword(false);
            setShowIntro(false);
          }}
        />
  ...
  return (
    <div className="min-h-dvh bg-slate-50 dark:bg-slate-950 transition-colors duration-200 flex flex-col">
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto relative pb-safe">{renderContent()}</div>
      ...
      {selectedClub && (
        <ChatWidget
          ...
          onEmergencyRequest={handleChatEmergency}
        />
      )}
      ...
      {selectedAdminBooking && (
        <AdminManageModal
          booking={selectedAdminBooking.booking}
          court={selectedAdminBooking.court}
          slot={selectedAdminBooking.slot}
          racketPrice={racketPrice}
          instructorPrice={instructorPrice}
          onClose={() => setSelectedAdminBooking(null)}
          onDelete={() => setBookings((p) => p.filter((b) => b.id !== selectedAdminBooking.booking.id))}
          onUpdate={(updates) =>
            setBookings((prev) =>
              prev.map((b) => (b.id === selectedAdminBooking.booking.id ? { ...b, ...updates } : b))
            )
          }
          onMoveStart={() => {
            setMovingBookingId(selectedAdminBooking.booking.id);
            setSelectedAdminBooking(null);
            setToastMessage('Seleziona il nuovo slot per spostare la prenotazione');
          }}
          onShowToast={setToastMessage}
        />
      )}
      ...
      {resultBookingId && (
        <ResultModal
          onClose={() => setResultBookingId(null)}
          onSave={(score, outcome) => {
            setBookings((prev) =>
              prev.map((b) => (b.id === resultBookingId ? { ...b, result: { score, outcome } } : b))
            );
            setResultBookingId(null);
            setSuccessMessage('Partita registrata!');
          }}
        />
      )}
      ...
      {editingCourt !== null && selectedClub && (
        <CourtEditorModal
          ...
          onSave={(data) => {
            if (data.id) {
              setCourts((prev) => prev.map((c) => (c.id === data.id ? { ...c, ...data, id: data.id! } : c)));
            } else {
              setCourts((prev) => [...prev, { ...data, id: Date.now() }]);
            }
      ...
      {(isCreatingClub || editingClub) && (
        <ClubEditorModal
          ...
          onSave={(data) => {
            if (data.id) {
              setClubs((prev) => prev.map((c) => (c.id === data.id ? { ...c, ...data, id: data.id! } : c)));
            } else {
              setClubs((prev) => [...prev, { ...data, id: Math.random().toString(36).substr(2, 9) }]);
            }
            setIsCreatingClub(false);
            setEditingClub(undefined);
          }}
...
function racksToPrice(rackets: number, racketPrice: number) {
  return rackets * racketPrice;
}