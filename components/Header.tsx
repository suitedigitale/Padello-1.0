...
import { Trophy, Sun, Moon, LogOut, User as UserIcon, MapPin, Shield, Bell, X } from 'lucide-react';
...
// Fixed Header component implementation and export
        ...
        <img src={logoPadello}
         ...
         className="h-11 w-auto object-contain"
          ...
          <div className="hidden sm:block">
            <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tighter">PADELLO</h1>
            {selectedClub && (
              <button 
          ...
          {user && (
            <div className="flex items-center gap-3">
              ...
              <div className="relative group" onClick={onEditProfile}>
                <button 
                  className="w-10 h-10 rounded-xl bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-800 overflow-hidden"
                >
                  <UserIcon size={20} />
                </button>
              </div>
      ...
      {isNotificationsOpen && (
        <div className="absolute top-full right-4 mt-2 w-80 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 p-4 animate-in fade-in slide-in-from-top-2">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => setIsNotificationsOpen(false)}><X size={16} /></button>
          </div>
          <div className="space-y-3 max-h-96 overflow-y-auto no-scrollbar">
            {playerNotifications.length === 0 ? (
              ...
              playerNotifications.map(n => (
                <div 
                  key={n.id} 
                  className={`p-3 rounded-xl border transition-colors cursor-pointer ${n.isRead ? 'bg-slate-50 dark:bg-slate-900/50 border-transparent' : 'bg-orange-50 dark:bg-orange-900/20 border-orange-100 dark:border-orange-800'}`}
                  onClick={() => onMarkNotificationAsRead(n.id)}
                >
                  <div className="font-bold text-xs text-slate-900 dark:text-white mb-1">{n.title}</div>
