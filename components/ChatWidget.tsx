...
export const ChatWidget: React.FC<ChatWidgetProps> = ({
  ...
  const handleSend = () => {
    ...
    const isSearchIntent =
      lowerInput.includes('cerca') ||
      lowerInput.includes('giocatore') ||
      lowerInput.includes('trova') ||
      lowerInput.includes('compagno');
    ...
    if (isSearchIntent) {
      botResponse(
        'Udite, udite! ⚔️ I miei messaggeri reali stanno cercando uno sfidante degno della vostra fama.',
        CONFIRM_IMG
      );
      if (user) {
  ...
  return (
    <>
      ...
      <button
        onClick={onToggle}
        className={`
          fixed right-5 z-[120] transition-all duration-500 hover:scale-110 active:scale-95
          ${isOpen ? 'opacity-0 pointer-events-none scale-75' : 'opacity-100 animate-padellotto-float'}
        `}
        style={{ bottom: 'calc(6.5rem + env(safe-area-inset-bottom))' }}
        aria-label="Apri chat Sir Padellotto"
      >
        <div className="relative w-20 h-20">
          <div className="absolute inset-0 rounded-full bg-orange-500/25 blur-xl animate-pulse" />
          <div className="relative w-20 h-20 rounded-full overflow-hidden ring-2 ring-white/10 shadow-[0_14px_30px_rgba(0,0,0,0.35)] bg-slate-900/70 backdrop-blur-sm">
            <img
              src={botAvatar}
              alt="Sir Padellotto"
              className="w-full h-full object-cover scale-[1.08] transition-transform duration-300 hover:scale-[1.14]"
            />
          </div>
        </div>
      </button>
      ...
      <div
        className={`
          fixed right-5 z-[130] w-[22rem] sm:w-96 bg-white dark:bg-slate-800 rounded-[2.5rem]
      ...
      >
        <div className={`p-5 flex justify-between items-center text-white relative overflow-hidden ${isManager ? 'bg-indigo-900' : 'bg-orange-600'}`}>
          <div className="flex items-center gap-3 z-10">
            <div className="w-12 h-12 rounded-2xl bg-white/95 p-0.5 overflow-hidden shrink-0 flex items-center justify-center shadow-lg border border-white/30">
              <img src={botAvatar} alt="Bot" className="w-full h-full object-cover scale-110" />
            </div>
            <div>
      ...
      <style>{`
        @keyframes padellotto-float {
          0%, 100% { transform: translateY(0); }
          50% { transform: translateY(-8px); }
        }

        .animate-padellotto-float {
          animation: padellotto-float 3s ease-in-out infinite;
        }
      `}</style>
    </>
  );
